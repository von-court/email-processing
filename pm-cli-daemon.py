...
def _resolve_bridge_password():
    """Resolve bridge password. Retries login if session expired."""
    env = os.environ.copy()
    env["PROTON_PASS_AGENT_REASON"] = AGENT_REASON
    ...
    err = cp.stderr.strip()
    log.warning("Session expired, re-logging in: %s", err)
    _login()
    cp = sp.run(
        [PASS_CLI_PATH, "item", "view", uri, "--output", "json"],
        capture_output=True, text=True, env=env,
    ...
    if cp.returncode != 0:
        raise RuntimeError(f"pass-cli failed (after re-login): {cp.stderr.strip()}")
    password = cp.stdout.strip()
    if not password:
        raise RuntimeError("empty password")
    return password
